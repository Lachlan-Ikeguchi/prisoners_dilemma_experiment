...
fn extract_description(content: &str) -> String {
    for line in content.lines() {
        let trimmed = line.trim();
        if trimmed.starts_with("-- will") || trimmed.starts_with("-- ") && !trimmed.starts_with("-- name:") {
...
/// Run a full tournament with all contestants
fn run_tournament(
    ...
    let mut results: HashMap<String, HashMap<String, (i32, i32)>> = HashMap::new();
    
    for i in 0..contestants.len() {
        for j in i..contestants.len() {
            ...
            results
                .entry(contestant1.name.clone())
                .or_default()
                .insert(contestant2.name.clone(), (total1, total2));
            
            if i != j {
                results
                    .entry(contestant2.name.clone())
                    .or_default()
                    .insert(contestant1.name.clone(), (total2, total1));
            }
        }
    }
...
fn print_results(
    ...
    let display_names: Vec<String> = contestants.iter().map(|c| {
        if c.name.len() > 12 {
    ...
    for (i, contestant) in contestants.iter().enumerate() {
        ...
        for opponent in contestants {
            if let Some(scores) = results.get(&contestant.name).and_then(|m| m.get(&opponent.name)) {
...
fn main() -> Result<()> {
    ...
    if contestants.is_empty() {
        eprintln!("No contestants found in directory: {}", args.contestants_dir);
        std::process::exit(1);
    }
